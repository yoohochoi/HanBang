...
<main>
	<section>
		...
		<form action="${ctx}/interestShareHouseDelete.do" method="get">
			<ul>
				<c:if test="${interestShareHouse eq 'value'}">
					<c:forEach var="interestShareHouse" items="${interestShareHouseList}" end="">
						<li>
							<input type="checkbox" id="interestShareHouseDeleteCheck" name="interestShareHouseDeleteCheck">
							<label class="hide" for="interestShareHouseDeleteCheck">셰어하우스 삭제</label>
							<a href="${ctx}/shareHouseDetail.do">
								<h4>${interestShareHouse.shareHouseTitle}</h4>
								<span>${interestShareHouse.shareHouseType}</span>
								<span>${interestShareHouse.shareHouseRoomEA}</span>
								<span>${interestShareHouse.shareHouseToiletEA}</span>
